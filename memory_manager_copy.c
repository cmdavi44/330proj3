...
enum hrtimer_restart my_hrtimer_callback( struct hrtimer *timer )
{
    ...
    totalPhysSize = totalPhys * 4;
    totalSwapSize = totalSwap * 4;
    accessedSize = accessedCnt * 4;
...
static int hrtimer_event_init_module(void)
{
    ...
    kt = ktime_set(0, timer_interval_ns);
...
// CURRENT ISSUES:
// - Test 3 is not working and the first WSS value for test 2 is incorrect
